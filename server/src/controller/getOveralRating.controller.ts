...
export const getOverallRatings = async (req: Request | any, res: Response| any) => {
    try {
        ...
        const ratingTotals = reviews.reduce(
            (acc, review) => ({
        ...
        const averageRatings = Object.fromEntries(
            Object.entries(ratingTotals).map(([key, total]) => [
                key,
