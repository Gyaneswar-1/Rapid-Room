...
export const createReview = async (req, res) => {
    try {
        ...
        const overalRating = Math.floor((cleanlinessRating +
            accuracyRating +
            checkInRating +
            communicationRating +
            locationRating +
        ...
        const [newReview, updatedHotel] = await prisma.$transaction(async (prisma) => {
            // Create the review
            ...
            const newOverallRating = Math.round((hotel.overalRating * hotel.totalReviews + overalRating) /
                newTotalReviews);
