 ...
 * @access Admin only
 */
export const admin_approveHost = async (req, res) => {
    ...
    try {
        ...
        return res.status(200).json(new ApiResponse(true, { user: updatedUser }, "User approved successfully"));
    }
    catch (error) {
        console.error("Error approving user:", error);
        return res.status(500).json(new ApiError(false, { error }, "Failed to approve user"));
    }
    finally {
