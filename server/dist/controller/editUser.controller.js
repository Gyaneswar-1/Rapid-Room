...
export const editUser = async (req, res) => {
    try {
        ...
        const imageUrl = await upLoadOnCloudinary(req.files &&
            ...
            ? req.files.profileImage[0].path
            : null);
        const { fullName, email, street, city, state, zipCode, country } = userData;
        ...
        res.status(200).json(new ApiResponse(true, result, "User updated successfully"));
    }
    catch (error) {
        res.status(500).json(new ApiError(false, { error }, "Failed to update user"));
    }
};
