...
export const editUser = async (req, res) => {
    try {
        ...
        if (!transaction) {
            return res
                .status(400)
                .json(new ApiError(false, {}, "Update Failed", "Failed to update user profile", 400));
        }
        const responseData = {
            user: {
        ...
        return res
            .status(200)
            .json(new ApiResponse(true, responseData, "Success", "User profile updated successfully", 200));
    }
    catch (error) {
        console.error("Error updating user data:", error);
        return res
            .status(500)
            .json(new ApiError(false, {}, "Server Error", "Server error while updating user profile", 500));
    }
    finally {
        prisma.$disconnect();
    }
